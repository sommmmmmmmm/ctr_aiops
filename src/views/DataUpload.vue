...
const validateData = async () => {
  ...
  const loading = ElMessage({
    message: '데이터를 검증하는 중... (데모 모드)',
    type: 'info',
    duration: 0
  })

  // Mock 모드로 직접 진행 (빠른 데모를 위해)
  ...
  // Mock 파일 ID 설정
  uploadedFileId.value = 'mock-file-' + Date.now()
...
const startTraining = async () => {
  try {
    ...
    if (!uploadedFileId.value) {
      ...
      console.log('Mock 파일 ID 자동 설정')
    }

    await ElMessageBox.confirm(
      '모델 학습을 시작하시겠습니까? 완료까지 약 2-3분 소요됩니다. (데모 모드)',
      '학습 시작 확인',
      {
    ...
    setTimeout(() => {
      ...
      router.push(`/training/${mockRunId}`)
    }, 1000)

  } catch (error) {
    if (error !== 'cancel') {
      console.error('Training error:', error)
      ElMessage.error('학습 시작에 실패했습니다.')
    }
